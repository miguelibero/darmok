...
function(darmok_process_shader)
    set(OPTIONS HEADER)
    set(ONE_VALUE_ARGS TYPE VARYING_DEF OUTPUT_DIR HEADER_INCLUDE_DIR HEADER_VAR_PREFIX OUT_VAR)
    ...
    if(DEFINED ARGS_HEADER_INCLUDE_DIR OR DEFINED ARGS_HEADER_VAR_PREFIX)
        set(ARGS_HEADER ON)
    endif()    
    if(${ARGS_HEADER})
        set(OUTPUT_EXT "h")
    ...
    foreach(SHADER_PATH ${ARGS_SHADERS})
        ...
        get_filename_component(SHADER_PATH_NAME_WE ${SHADER_PATH} NAME_WLE)
        ...
        if(DEFINED ARGS_HEADER_VAR_PREFIX)
            set(SHADER_HEADER_VAR_PREFIX ${ARGS_HEADER_VAR_PREFIX})
        else()
            set(SHADER_HEADER_VAR_PREFIX ${SHADER_PATH_NAME_WE})
            string(REPLACE "." "_" SHADER_HEADER_VAR_PREFIX ${SHADER_HEADER_VAR_PREFIX})
        endif()
        foreach(PROFILE ${PROFILES})
            _bgfx_get_profile_ext(${PROFILE} PROFILE_EXT)
            set(OUTPUT_FILE ${SHADER_PATH_NAME_WE}.${PROFILE_EXT}.${OUTPUT_EXT})
        ...
        if(${ARGS_HEADER})
            set(COMBINED_OUTPUT ${ARGS_OUTPUT_DIR}/${SHADER_PATH_NAME_WE}.${OUTPUT_EXT})
            list(APPEND OUTPUTS ${COMBINED_OUTPUT})
            string(REPLACE ";" "$<SEMICOLON>" GENERATOR_COMBINED_INCLUDES "${SHADER_COMBINED_INCLUDES}")
            list(APPEND SHADER_COMMANDS COMMAND ${CMAKE_COMMAND}  -D HEADER_INCLUDES="${GENERATOR_COMBINED_INCLUDES}" -D OUTPUT="${COMBINED_OUTPUT}" -P ${CMAKE_CURRENT_SOURCE_DIR}/CMake/generateCombinedHeader.cmake)
        endif()
    ...
    endforeach()
    if(NOT ${ARGS_ARGS_OUT_VARSOURCES_VAR} STREQUAL "")
